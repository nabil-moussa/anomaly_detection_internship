...
def _detect_nan_boundaries(signal, min_points):
    ...
    for s, e in zip(starts[:n], ends[:n]):
        ...
        if length > min_points:
            cycles.append((s, true_end, length))
    return cycles
...
def detect_cycles_with_fallback(signal, raw_df, cols, min_points=5,
    ...
    if forced_period is not None and forced_period > min_points:
        ...
        for s in range(0, len(signal) - forced_period, forced_period):
            e = s + forced_period - 1
            segment  = signal.iloc[s:e + 1]
            nan_frac = segment.isna().mean()
            if nan_frac < 0.10:   # skip any stride that overlaps a separator
                stride_cycles.append((s, e, forced_period))
...
def score_single_cycle(nc, tmpl, template_median=None, max_shift_frac=0.10):
    ...
    for w in range(tmpl['n_windows']):
        ...
        zm = abs(om - tmpl['mean_of_means'][w]) / (tmpl['std_of_means'][w] + eps)
        zs = abs(os_ - tmpl['mean_of_stds'][w])  / (tmpl['std_of_stds'][w]  + eps)
        comb = (zm + zs) / 2.0
        scores.append(comb)
        details.append({'window_idx': w, 'obs_mean': om, 'obs_std': os_,
                        'exp_mean': tmpl['mean_of_means'][w], 'exp_std': tmpl['mean_of_stds'][w],
...
def run_pipeline_for_group(df, group_cols, group_period, group_id, min_cycles=3):
    ...
    if seg_method == "NaN boundary" or seg_method == "Consistent NaN gaps":
        try:
            full_cycles, _ = detect_cycles_with_fallback(
                corr_full, df_full, group_cols,
                forced_period=int(np.median([l for _,_,l in train_cycles]))
            )
        except RuntimeError as e:
            print(f"  ✗ Full-stream segmentation failed: {e}")
            return None
    else:
        ...
        for s in range(0, len(corr_full) - test_period, test_period):
            e        = s + test_period - 1
            segment  = corr_full.iloc[s:e + 1]
            nan_frac = segment.isna().mean()
            if nan_frac < 0.10:   # skip strides that overlap a separator
    ...
    for cid, train_idx_set in train_template_idx_sets.items():
        remapped = set()
        for ti in train_idx_set:
            train_start = train_cycles[ti][0]
            # find the full-stream cycle whose start matches closest
            best_fi = min(range(len(full_cycles)),
                        key=lambda fi: abs(full_cycles[fi][0] - train_start))
            remapped.add(best_fi)
        template_global_idx_sets[cid] = remapped
        all_template_indices.update(remapped)

    all_normalised = [normalize_cycle(corr_full, s, e, target_length)
                    for s, e, _ in full_cycles]

    # test = cycles not used as templates and successfully normalised
