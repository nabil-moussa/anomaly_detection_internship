...
class Trainer:
    ...
    def fit(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            # KL annealing: linearly ramp over first 50% of epochs
            kl_anneal = min(1.0, (epoch + 1) / (0.5 * self.n_epochs)) if self.use_vae else 0.0
            ...
            for x, y in train_loader:
                ...
                if self.use_vae:
                    preds, recons, _, kl_loss = self.model(x)
                else:
                ...
                recon_loss    = torch.sqrt(self.recon_criterion(x_recon, recons))

                if self.use_vae:
                    loss = forecast_loss + recon_loss + kl_anneal * self.kl_weight * kl_loss
                else:
            ...
            forecast_val_loss = recon_val_loss = total_val_loss = "NA"
            if val_loader is not None:
            ...
            if epoch % self.print_every == 0:
                s = (
                    f"[Epoch {epoch + 1}] "
                    f"forecast={forecast_epoch_loss:.5f}  "
                    f"recon={recon_epoch_loss:.5f}  "
                    f"total={total_epoch_loss:.5f}"
                )
                if val_loader is not None:
                    s += (
                        f"  |  val_forecast={forecast_val_loss:.5f}  "
                        f"val_recon={recon_val_loss:.5f}  "
                        f"val_total={total_val_loss:.5f}"
                    )
                s += f"  [{epoch_time:.1f}s]"
                print(s)
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for x, y in data_loader:
                ...
                if self.use_vae:
                    preds, recons, _, __ = self.model(x)
                else:
                    preds, recons = self.model(x)
                ...
                forecast_losses.append(torch.sqrt(self.forecast_criterion(y, preds)).item())
                recon_losses.append(torch.sqrt(self.recon_criterion(x_recon, recons)).item())
