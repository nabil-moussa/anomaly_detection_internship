...
def pairwise_periods(df, cols, train_end, window):
    """
    For each sensor pair, compute the dominant period of their bilateral
    rolling |correlation| signal. Returns a (n×n) period matrix and
    a (n×n) confidence matrix.
    """
    n   = len(cols)
...
def group_sensors_by_period(cols, period_matrix, conf_matrix,
                             ...
                             period_tol=0.20):
    """
    Cluster sensors into groups based on:
      1. They must have mean |corr| >= min_corr with at least one other sensor
      2. Their pairwise dominant period must agree within period_tol (20%)

    Uses hierarchical clustering on period distance.
    Returns list of groups (each group is a list of sensor names).
    Sensors that don't meet the correlation threshold are flagged as ungroupable.
    """
    n = len(cols)
    ...
    # Filter: a group is only valid if it has >= 2 sensors with mean pairwise correlation >= min_corr
    valid_groups   = []
    invalid_sensors = []
...
def score_correlation_strength(mean_corr):
    """
    Map mean |correlation| to [0, 1].
    0.0 → 0.0, 0.15 → 0.3, 0.30 → 0.6, 0.50+ → 1.0
    """
